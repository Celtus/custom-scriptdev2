...
bool TELE::IsAllowedToTeleport(Player * const player) const
{
    ...
    error_log("NPC Teleport: Invalid flag (category: %u).", GetCatID());
    return false;
}
...
void LoadNpcTele(QueryResult *result)
{
    ...
    if (result)
    {
        outstring_log( "Script: Loading NPC Teleport Data...");
        ...
        outstring_log("Script: >> Loaded %u NPC Teleport Locations", nbDest);
    } else outstring_log("Script: >> Loaded 0 NPC Teleport Locations - Check for errors");
    delete result;
}
